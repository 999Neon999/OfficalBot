...
# BEST 1M CONFIGURATION (from optimization)
AI_THRESHOLD = 0.85
BASE_TARGET_PCT = 0.015  # 2.0%
STOP_LOSS_PCT = 0.010   # 5.0%
SQUEEZE_THRESHOLD = 0.0016
...
for sym in df_raw['symbol'].unique():
    ...
    for ts, row in df_feat[df_feat['prob'] >= AI_THRESHOLD].iterrows():
        try:
            ...
            obs_limit = min(len(c_arr)-loc-1, HOLD_TIME)
...
# Calculate trading days (1m bars: ~375 per day)
